...
for the Teknowsolutions Invoice Automation SOW template (WhatsApp-based
vehicle invoice intake, AI validation, storage, review, and reporting).
"""
...
MILESTONE_DELIVERABLES = [
    "Parametrization rules and validation criteria defined with Client. Data model design "
    "(vehicles, invoices, validation rules). Architecture diagram for the WhatsApp intake "
    "and AI validation workflow. UI/UX wireframes for the review and reporting panels.",
    "WhatsApp Business API webhook to receive invoice images/PDFs. AI-based data extraction "
    "and automated validation of each invoice against the agreed parametrization criteria "
    "(vehicle identification, amount, vendor, date, required fields), with automatic "
    "flagging of invoices that fail validation.",
    "PostgreSQL schema for vehicles, invoices, and validation rules. Backend logic to group "
    "stored invoices by their related vehicle and expose the review/correction API.",
    "Secure login portal displaying invoices grouped by vehicle, with manual correction/edit "
    "capability for flagged or misparsed invoices. Deployed to staging.",
    "Reporting module to visualize one or multiple vehicles with all their related invoices "
    "(filters, totals, export). End-to-end testing, bug fixes, production deployment, and "
    "final launch sign-off.",
]

_SCOPE_ROWS = [
    ("WhatsApp Intake",
     "WhatsApp Business API webhook to receive invoice images/PDFs submitted by users.",
     "API / Webhooks"),
    ("AI Validation Engine",
     "Automated extraction and validation of invoice data against configurable "
     "parametrization criteria (vehicle identification, amount, vendor, date, required "
     "fields). Invoices that fail validation are flagged for manual review.",
     "Backend / Automation (AI)"),
    ("Database & Backend",
     "Relational data model for vehicles, invoices, and validation rules. Core business "
     "logic and secure API layers to store and group validated invoices.",
     "Cloud Infrastructure"),
    ("Review & Correction WebApp",
     "Private panel with secure login showing invoices grouped by their related vehicle, "
     "with manual correction/edit capability for flagged or misparsed records.",
     "Web (Responsive)"),
    ("Reporting Module",
     "Reporting interface to visualize one or multiple vehicles together with all their "
     "related invoices, including filters, totals, and export.",
     "Web (Responsive)"),
]

_OUT_OF_SCOPE = [
    "third-party service fees (Meta WhatsApp API costs, AI/LLM API usage costs, cloud "
    "hosting, Google Workspace licenses);",
    "content creation, copywriting, or migration of historical/pre-existing invoice data;",
    "native mobile applications (iOS/Android apps on stores);",
    "accounting or ERP system integrations not explicitly listed in Section 2.1;",
    "ongoing support or maintenance after the warranty period (covered separately under a "
    "Support & Maintenance SOW);",
    "physical scanning or digitization of paper invoices.",
]

_CLIENT_RESPONSIBILITIES = [
    "provide timely feedback within three (3) business days of each deliverable submission;",
    "supply all required content, parametrization criteria, credentials, and third-party "
...
_TECH_STACK_ROWS = [
    ("Frontend (Review WebApp & Reports)", "React, Tailwind, Vite",
     "Modular, fast, and optimized production builds."),
    ("Automation / AI Extraction", "OpenAI API, Custom Validation Engine",
     "AI-assisted data extraction and rule-based validation."),
    ("Backend / API Layer", "Python, FastAPI",
     "High-performance, asynchronous REST API architecture."),
    ("Database & Data", "PostgreSQL",
     "Relational storage for vehicles, invoices, and validation rules."),
    ...
    ("Integrations", "WhatsApp Business API",
     "Official Meta integration for invoice intake."),
]
...
def generate_invoice_sow_pdf(data: InvoiceSOWData) -> bytes:
    """Generate the full Invoice Automation SOW PDF and return its bytes."""
    buffer = BytesIO()
...
def _build_story(data: InvoiceSOWData, S: dict) -> list:
    story = []
    ...
    story += [
        ...
        Paragraph(
            ...
            f'<b>{data.project_description}</b> (the "Project"), a WhatsApp-based invoice '
            f'intake, validation, and reporting automation system described herein.',
            S["body"],
        ),
        Paragraph(
    ...
    story += [
        ...
        Paragraph(
            ...
            "custom parametrization changes, Meta WhatsApp account suspensions, or AI/LLM "
            "provider API downtime/failures; (b) hosting or infrastructure failures outside "
            "Consultant's control; or (c) new feature requests.",
            S["body"],
        ),
        Spacer(1, 0.12 * inch),
    ]
...
def _signature_block(data: InvoiceSOWData, S: dict) -> Table:
